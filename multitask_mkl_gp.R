require(rstan)
require(doMC)
source("load_data.R")
#source("load_sub1_data.R")
source("load_lb_data.R")

sub1b=T

# Compile STAN model
sm=stan_model("comb_therapy_models/gp_multitask_mkl.stan")

# Get cell lines
cls=levels(train$CELL_LINE)

# Cell line distance matrices
sqDist=setNames( foreach(g=dist) %do% ( g[cls,cls]^2 ), names(dist) )

#require(stringdist)
#sdm=as.matrix(stringdistmatrix(levels(train$COMPOUND_A),method="cosine"))
#dimnames(sdm)=list(levels(train$COMPOUND_A),levels(train$COMPOUND_A))

drugs=levels(train$COMPOUND_A)

# Get drug targets list
pathways=read.csv("DREAM CHALLENGE TABLE DRUGS-TARGETS LOUKIA.csv",row.names = 1,check.names = F)
pathways=pathways[,colSums(pathways)>1]
ldrugs=do.call(rbind,strsplit(rownames(pathways),"-"))[,2]
#sort(drugs[ !drugs %in% ldrugs])
#sort( ldrugs[ ! ldrugs %in% drugs ] )
rownames(pathways)=ldrugs
pathways=pathways[drugs,]
pathways=as.matrix(pathways)

# Convert pathway network into distances on graph
require(sna)
graph_dist=geodist(pathways,count.paths = F, inf.replace = 200)$gdist[1:nrow(pathways),1:nrow(pathways)]/2
graph_dist=graph_dist/median(graph_dist[upper.tri(graph_dist)])

# Load PCA imputed monotherapy data
co=read.table("pca_imputed_mono.txt",header=T)
mono_dist=as.matrix(dist(t(co)))
mono_dist=mono_dist/median(mono_dist[upper.tri(mono_dist)])

# Distance matrices for the drugs
sqDist_dr=list( pathways=graph_dist^2, mono=mono_dist[drugs,drugs]^2 ) # sdm

if (sub1b) {
  sqDist=sqDist[c("cnv","mut")]
  sqDist_dr=sqDist_dr["pathways"]
}

dat=list(N=nrow(train), Ntest=nrow(test), y=train$SYNERGY_SCORE, C=length(cls), D=length(levels(train$COMPOUND_A)), P=length(sqDist), sqDist_cl=sqDist, P_dr=length(sqDist_dr), sqDist_dr=sqDist_dr, cellLines=as.numeric(train$CELL_LINE), cellLinesTest=as.numeric(test$CELL_LINE), drugA=as.numeric(train$COMPOUND_A), drugB=as.numeric(train$COMPOUND_B), drugATest=as.numeric(test$COMPOUND_A), drugBTest=as.numeric(test$COMPOUND_B) )

# Run six repeats
reruns=foreach(i=1:6) %dopar% optimizing(sm, data=dat,verbose=T,as_vector=F)
save(reruns, file="multitask_mkl_gp_sub1B.RData")

# Get likelihood of each fit
likelihoods=foreach(r=reruns, .combine = c) %do% r$value
barplot(-likelihoods)

o=reruns[[ which.max(likelihoods) ]]

# o$value
require(gridExtra)
do.call(grid.arrange , c(foreach(r=reruns[order(likelihoods)]) %do% {
  ggplot(data.frame(x=names(sqDist), y=sqrt(r$par$eta_sq_cl)), aes(x,y)) + geom_bar(stat="identity") + ggtitle(paste0("LL: ",format(r$value, digits = 3), " Score:")) + theme_bw(base_size=14) + ylab("importance") + xlab("") + ylim(0,2.3) } , nrow=1))

do.call(grid.arrange , c(foreach(r=reruns[order(likelihoods)]) %do% {
  ggplot(data.frame(x=c("pathways","mono_therapy"), y=sqrt(r$par$eta_sq_dr)), aes(x,y)) + geom_bar(stat="identity") + ggtitle(paste0("Likelihood: ",format(r$value, digits = 3))) + theme_bw(base_size=14) + ylab("importance") + xlab("") + ylim(0,1) } , nrow=1))

plot(x[traininds],y[traininds])
points( x[testinds], o$par$ytest, col="red", pch=16 )

sqrt(mean( (test$SYNERGY_SCORE-o$par$ytest)^2 ))

scores=foreach(o=reruns, .combine = c) %do% get_score(o$par$ytest)

log_sum_exp=function(x) log(sum(exp(x - max(x)))) + max(x)

weighting=exp(likelihoods - log_sum_exp(likelihoods))

preds=foreach(o=reruns, .combine = cbind) %do% o$par$ytest

weighted_pred=as.numeric(preds %*% weighting)
get_score(weighted_pred, test)

scores[which.max(likelihoods)]

ggplot(data.frame(likelihoods, scores), aes(likelihoods, scores)) +geom_point(size=3) + theme_bw(base_size = 16) + ylab("Score") + xlab("Log likelihood")

dfsub=df[df$n>2,]
ggplot(dfsub, aes(as.factor(n), pearson)) + geom_boxplot(outlier.shape = NA) + xlab("# cell lines in combination") + ylab("Pearson correlation") + geom_point(position = position_jitter(width = .5), size=3, alpha=.5) + theme_bw(base_size = 15)

names(o$par$eta_sq_cl)=names(sqDist)
barplot(sqrt(o$par$eta_sq_cl),ylab="importance")

names(o$par$eta_sq_dr)=c("pathways","mono_therapy")
barplot(sqrt(o$par$eta_sq_dr),ylab="importance")

N=nrow(train)
o$par$sigma_sq
SN=o$par$Sigma_no_noise

ytrain = SN %*% ( solve(SN + diag(N) * o$par$sigma_sq, dat$y - o$par$mu) ) + o$par$mu
train$err= (ytrain-dat$y)^2

require(dplyr)
errs=train %>% group_by(COMBINATION_ID) %>% summarize(err=mean(err), mean=mean(SYNERGY_SCORE))
setDF(errs)
a=as.data.frame(errs[1:167,])

plot( a$mean, sqrt(a$err ), xlab="mean synergy", ylab="training RMSE", pch=16 ) 
#text( a$mean, sqrt(a$err ), a$COMBINATION_ID)
a$cv=sqrt(a$err)/ abs(a$mean) 
a=a[order(a$cv),]
cv=a$cv
names(cv)=a$COMBINATION_ID
cv=cv[c(1:20,length(cv)-20+(1:20))]
barplot(cv, horiz = T, xlab="coefficient of variation", las=2)

b=a[c(1:20,nrow(a)-20+(1:20)),]
b$COMBINATION_ID=factor(b$COMBINATION_ID,b$COMBINATION_ID)
ggplot(b, aes(x=COMBINATION_ID, y=cv) ) + geom_bar(stat = "identity")+coord_flip() + scale_y_log10()

str(o)
sub1=data.frame(CELL_LINE=test$CELL_LINE, COMBINATION_ID=test$COMBINATION_ID, PREDICTION=o$par$ytest)

setwd("sub1_partA")
write.csv(sub1,file="prediction.csv",row.names = F, quote=F)

sub1conf=data.frame(COMBINATION_ID=a$COMBINATION_ID, CONFIDENCE=1-pnorm( -abs(a$mean) / sqrt(a$err) )*2 )
write.csv(sub1conf,file="combination_priority.csv",row.names = F, quote=F)

system("zip sub1a.zip *.csv")
setwd("..")