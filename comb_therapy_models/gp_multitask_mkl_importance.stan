...
data {
  ...
  real<lower=0> eta_sq_cl_ge; // GP1 signal variance
  real<lower=0> inv_rho_sq_ge; // squared length scale GP 1
...
model {
  ...
  for (i in 1:N) {
      ...
      temp[P+1] <- eta_sq_cl_ge * exp(-rho_sq_ge * dot_self( ge[ cellLines[i] ] - ge_test ) );
      for (p in 1:P_dr)
